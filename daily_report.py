...
def main():
    ...
    # Sidebar Menu
    menu = ["Metrics Dashboard", "User Leaderboard", "Content Analysis", "Analysis Summary", "Curriculum Overview", "Mock Interviews"] # Added "Mock Interviews"
    ...
    elif choice == "Mock Interviews": # Added condition for "Mock Interviews"
        display_mock_interviews(engine) # Call new function


# --- NEW FUNCTION FOR MOCK INTERVIEWS ---
def display_mock_interviews(engine):
    ...
    # Metric 1: Total Mock Interviews (already implemented)
    ...
    # Metric 2: Average Feedback Score (if numeric)
    avg_feedback_query = text("SELECT AVG(overall_feedback::numeric) FROM interview_sessions WHERE overall_feedback IS NOT NULL AND pg_typeof(overall_feedback) = 'numeric'") # Added type check and NULL handling
    try:
        with engine.connect() as conn:
            avg_feedback_result = conn.execute(avg_feedback_query).scalar()
            avg_feedback = round(avg_feedback_result, 2) if avg_feedback_result else "N/A" # Round to 2 decimal places
    except Exception as e:
    ...
    try:
        with engine.connect() as conn:
            ...
            if not recent_interviews_df.empty:
                ...
                st.dataframe(recent_interviews_df[['Session ID', 'First Name', 'Last Name', 'Feedback', 'Interview Date']], height=500)
            else:
    ...
    selected_range_charts = st.selectbox("Select Time Range for Feedback Charts", list(time_ranges_charts.keys()), key="feedback_time_range_selector") # Unique key
    ...
    # Chart 1: Feedback Score Trend Over Time (Line Chart) - if numeric feedback
    feedback_trend_query = text("""
        SELECT DATE(created_at) as interview_date, AVG(overall_feedback::numeric) as avg_feedback
        FROM interview_sessions
        WHERE overall_feedback IS NOT NULL AND pg_typeof(overall_feedback) = 'numeric' AND created_at >= :start_time
    ...
    try:
        with engine.connect() as conn:
            ...
            if not feedback_trend_df.empty:
                ...
                st.line_chart(feedback_trend_df, y="Average Feedback Score", height=300, use_container_width=True) # Line chart
            else:
    ...
    # Chart 2: Feedback Score Distribution (Bar Chart) - if feedback is categorical or numeric (let's assume categorical for now, or distinct values if numeric)
    ...
    try:
        with engine.connect() as conn:
            ...
            if not feedback_distribution_df.empty:
                ...
                st.bar_chart(feedback_distribution_df, height=300, use_container_width=True) # Bar chart
            else:
